...
public:
  ...
  {
    SLog(cc::util::DEBUG) << "FILE FILTER '" << filters_.fileFilter << "'";
    SLog(cc::util::DEBUG) << "DIR FILTER '" << filters_.dirFilter << "'";
  }
...
public:
  bool shouldSkip(const std::string& filePath_) const
  {
    ...
    {
      skip = shouldSkipByFilter(_fileFilter, cc::util::getFilename(filePath_));
    }

    if (!skip && !_filters.dirFilter.empty())
    {
      skip = shouldSkipByFilter(_dirFilter, cc::util::getPath(filePath_));
    }
...
private:
      ...
      const std::string&      filePath_)
  {
    ...
    {
      SLog() << "Oooops, we've got an exception! what(): " << err.what();
  ...
  _db(db_)
{
  ...
  if (!config_.count("searchdir"))
  {
    ...
    OptionResult r(_db->query<model::Option>(
        OptionQuery::key == "searchIndexDir"));
...
void SearchServiceHandler::search(
  cc::service::core::SearchResult& _return,
  const SearchParams& params_)
{
  ...
  try
  {
    ...
    SLog(util::INFO) << "Search time: " << dur.count() << " milliseconds.";
  }
  catch (const ServiceProcess::ProcessDied&)
  {
    SLog(util::CRITICAL) << "Java search service died! Terminating server...";
    ::abort();
  }
}
...
void SearchServiceHandler::searchFile(
    ...
    const SearchParams&     params_)
{
  SLog(util::INFO) << "Search for file: query = " << params_.query.query;
  ...
  try
  {
    ...
    for (const auto& f : r)
    {
      ...
      core::BasicFileInfo info;
      info.file.fid = std::to_string(f.id);
  ...
  {
    SLog(util::ERROR) << "Regexp error: " << err.what();
  ...
  const SearchSuggestionParams& params_)
{
  ...
  try
  {
    ...
    SLog(util::INFO) << "Suggest time: " << dur.count() << " milliseconds.";
  }
  catch (const ServiceProcess::ProcessDied&)
  {
    SLog(util::CRITICAL) << "Java search service died! Terminating server...";
