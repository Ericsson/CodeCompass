...
namespace core
{
  ...
  void ProjectServiceHandler::getFileInfo(FileInfo& _return, const FileId& file)
  {
    transaction ([&, this](){
      try
      {
        ...
        {
          BOOST_LOG_TRIVIAL(error) << "invalid file ID: " << file.fid;
  ...
  {    
    BOOST_LOG_TRIVIAL(debug)  <<"getFileStat requested for file id:" << file.fid;
    FileInfo fileInfo;
...
//          {
//            BOOST_LOG_TRIVIAL(error) << "invalid project ID: " << project.prid;
//
//            InvalidId ex;
...
//          auto pathsQuery = _db->query<model::Option>(query::key == "input");
//
...
//          }
  ...
  {      
    BOOST_LOG_TRIVIAL(debug)  <<"getRootFiles requested ";
    transaction ([&, this](){
        typedef odb::result<model::File> fileResult;
        typedef odb::query<model::File> query;
        try{
          fileResult r (_db->query<model::File> (query::parent.is_null()));
          model::File f;
          for (fileResult::iterator i(r.begin()); i!=r.end();++i){    
            i.load(f);
            if (f.path != "."){
              BOOST_LOG_TRIVIAL(debug)
                <<"getRootFiles requested fileName:"<<f.path<<"fid:"<<f.id;
  ...
  void ProjectServiceHandler::getChildFiles(std::vector<FileInfo> & _return, const FileId& file)
  {
    ...
    BOOST_LOG_TRIVIAL(debug)  <<"getChildfFiles requested ";
    ...
    transaction ([&, this](){
        try{
          ...
          for (fileResult::iterator i(r.begin()); i!=r.end();++i){    
            i.load(f);
            BOOST_LOG_TRIVIAL(debug)  <<"child filePath:"<<f.path;
            FileInfo fileInfo=makeFileInfo(f);
            _return.push_back(fileInfo); 
          }
        }
        catch (odb::exception &odbex){  
          BOOST_LOG_TRIVIAL(error) << "datasource exception occured: " << odbex.what();
  ...
  {    
    ...
    {
      BOOST_LOG_TRIVIAL(debug) << "this is a directory";
    ...
    case model::File::PSFullyParsed:
      ...
      break;
    default:
      BOOST_LOG_TRIVIAL(error) << "Unknown parse status!";
      fileInfo.parseStatus = FileParseStatus::Nothing;
      break;
    }
  ...
  void ProjectServiceHandler::searchFile(std::vector<FileInfo> & _return, const std::string& searchText, const bool onlyFile) {
    BOOST_LOG_TRIVIAL(debug) <<"searchFile requested "<<searchText;
    typedef odb::result<model::File> fileResult;
    typedef odb::query<model::File> query;
    transaction ([&, this](){
        try{
          ...
          for (fileResult::iterator i(r.begin()); i!=r.end();++i){    
            i.load(f);
            BOOST_LOG_TRIVIAL(debug)  <<"search filePath:"<<f.path;
            FileInfo fileInfo=makeFileInfo(f);
            _return.push_back(fileInfo); 
          }
        }
  ...
  void ProjectServiceHandler::checkDbAndThrow()
  {
    if (!_db)
    {
      BOOST_LOG_TRIVIAL(error) << "invalid database in workspace service";
    ...
    BOOST_LOG_TRIVIAL(debug) << "database is valid in workspace service";
  }
