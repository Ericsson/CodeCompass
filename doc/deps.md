...
| `WITH_PLUGIN` | The names of the plugins to be built. Possible values are **cpp**, **cpp_reparse**, **dummy**, **git**, **metrics**, **search**. The metrics and search plugins are fundamental, they will be compiled even if not included. Keep in mind that the cpp_reparse plugin cannot be compiled without the cpp plugin. `WITH_PLUGIN` **cannot** be used together with `WITHOUT_PLUGIN`. Example: `-DWITH_PLUGIN="cpp;git"` This will compile the cpp, git, metrics and search plugins. |
| `WITHOUT_PLUGIN` | The names of the plugins to be skipped at build. Possible values are **cpp**, **cpp_reparse**, **dummy**, **git**, **metrics**, **search**. The metrics and search plugins are fundamental, they will be compiled even if included. Keep in mind that the cpp_reparse plugin cannot be compiled without the cpp plugin. `WITHOUT_PLUGIN` **cannot** be used together with `WITH_PLUGIN`. Example: `-DWITHOUT_PLUGIN="dummy;git;search"` This will compile the cpp, cpp_reparse, metrics and search plugins. |