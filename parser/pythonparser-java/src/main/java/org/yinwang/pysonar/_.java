...
public class _ {
    ...
    public static String readFile(@NotNull String path) {
        ...
        if (content == null) {
            System.out.println("\n[IO Error] -> _.java.readFile(" + path + "): Unable to read file.");
            return null;
        } else {
    ...
    public static void die(String msg, Exception e) {
        ...
        if (e != null) {
            e.printStackTrace();
            PPLogger.severe(e.getMessage());
        }

        Analyzer.self.terminateExecution();
    }
    ...
    public static String locateTmp(String file) {
        ...
        return makePathString(tmpDir, "pysonar2_" + userName + "_" + Analyzer.self.sid, file);
    }
    ...
    public static String commonPath(Set<String> paths) {
        return commonPath(paths.toArray(new String[paths.size()]));
    }

    public static String commonPath(String... paths) {
        String separator = File.separator;
        if(separator.equals("\\")) {
            separator = "\\\\"; // pass escaped slash to the regexp
        }
        ...
        for (int i = 0; i < paths.length; i++) {
            List<String> folder = new ArrayList<String>(Arrays.asList(paths[i].split(separator))); //split on file separator
            if(folder.get(folder.size()-1).endsWith(".py")) {
                folder.remove(folder.size()-1);
            }
            folders.add(folder);
        }
        for (int j = 0; j < folders.get(0).size(); j++) {
            String thisFolder = folders.get(0).get(j); //grab the next folder name in the first path
            boolean allMatched = true; //assume all have matched in case there are no more paths
            for (int i = 1; i < folders.size() && allMatched; i++) { //look at the other paths
                if (folders.get(i).size() < j) { //if there is no folder here
                    allMatched = false; //no match
                    break; //stop looking because we've gone as far as we can
                }
                //otherwise
                allMatched &= folders.get(i).get(j).equals(thisFolder); //check if it matched
            }
