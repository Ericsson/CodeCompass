...
public class Parser {
    ...
    // we use a modified dump_python.py
    ...
    public Node convert(Object o) {
        ...
        if (type.equals("alias")) {         // lower case alias
            ...
            names.add(new Name(qname, NameType.LOCAL, file, start, end,
                    (int) startLine.doubleValue(),
                    (int) offset.doubleValue(),
                    (int) endLine.doubleValue(),
                    (int) end_offset.doubleValue())
            );
            Name asname = map.get("asname") == null ? null : (Name) convert(map.get("asname"));
        ...
        if (type.equals("ExceptHandler")) {
            Node exception = convert(map.get("original_type"));
        ...
        if (type.equals("FunctionDef") || type.equals("Lambda")) {
            ...
            List<Node> decorators = convertList(map.get("decorator_list"));
...
//            }
            ...
            Object varargObj = map.get("vararg_name");
            if (varargObj instanceof Map) {
                Map varargsMap = (Map) varargObj;
                String argsId = (String) varargsMap.get("id");
                int argsStart = (int) ((Double) varargsMap.get("start")).doubleValue();
                int argsEnd = (int) ((Double) varargsMap.get("end")).doubleValue();
                int argsLineStart = (int) ((Double) varargsMap.get("start_lineno")).doubleValue();
                int argsLineEnd = (int) ((Double) varargsMap.get("end_lineno")).doubleValue();
                int argsOffset = (int) ((Double) varargsMap.get("col_offset")).doubleValue();
                int argsEndOffset = (int) ((Double) varargsMap.get("end_col_offset")).doubleValue();
                vararg = new Name(argsId, NameType.LOCAL, file, argsStart, argsEnd,
                        argsLineStart, argsOffset, argsLineEnd, argsEndOffset);
            } else if (varargObj instanceof String) {
                // empty
            }
...
//            }
            ...
            Object kwargObj = map.get("kwarg_name");
            if (kwargObj instanceof Map) {
                Map kwargMap = (Map) kwargObj;
                String argsId = (String) kwargMap.get("id");
                int argsStart = (int) ((Double) kwargMap.get("start")).doubleValue();
                int argsEnd = (int) ((Double) kwargMap.get("end")).doubleValue();
                int argsLineStart = (int) ((Double) kwargMap.get("start_lineno")).doubleValue();
                int argsLineEnd = (int) ((Double) kwargMap.get("end_lineno")).doubleValue();
                int argsOffset = (int) ((Double) kwargMap.get("col_offset")).doubleValue();
                int argsEndOffset = (int) ((Double) kwargMap.get("end_col_offset")).doubleValue();
                kwarg = new Name(argsId, NameType.LOCAL, file, argsStart, argsEnd,
                        argsLineStart, argsOffset, argsLineEnd, argsEndOffset);
            } else if (kwargObj instanceof String) {
                // empty
            }

            return new FunctionDef(name, args, body, defaults, decorators, vararg, kwarg, file, start, end);
        }
        ...
        if (type.equals("Import")) {
            ...
            for (Alias alias : aliases) {
                alias.name = segmentQname(
                        alias.name.get(0).id,
                        alias.name.get(0).start,
                        alias.name.get(0).end,
                        alias.name.get(0).getLineStart(),
                        alias.name.get(0).getOffset(),
                        alias.name.get(0).getLineEnd(),
                        alias.name.get(0).getEndOffset(),
                        alias.name.get(0).parent,
                        alias.name.get(0).getAstType());
            }
        ...
        if (type.equals("ImportFrom")) {
            ...
            Map<String, Object> module = (Map<String, Object>) map.get("module");
            String module_name = (String) module.get("name");
            int module_start = ((Double) module.get("start")).intValue();
            int module_end = ((Double) module.get("end")).intValue();

            Double module_startLine = ((Double) map.get("start_lineno") == null)
                    ? -1.0 : (Double) map.get("start_lineno");
            Double module_endLine = ((Double) map.get("end_lineno") == null)
                    ? -1.0 : (Double) map.get("end_lineno");
            Double module_offset = ((Double) map.get("col_offset") == null)
                    ? -1.0 : (Double) map.get("col_offset");
            Double module_end_offset = ((Double) map.get("end_col_offset") == null)
                    ? -1.0 : (Double) map.get("end_col_offset");

            List<Name> moduleSeg = module == null
                    ? null
                    : segmentQname(
                            module_name,
                            module_start,
                            module_end,
                            module_startLine.intValue(),
                            module_offset.intValue() + "from ".length(),
                            module_endLine.intValue(),
                            module_end_offset.intValue(),
                            names.get(0).name.get(0).parent, // just a hack, nothing else.
                            names.get(0).getAstType());
        ...
        if (type.equals("Raise")) {
            Node exceptionType = convert(map.get("original_type"));
    ...
    List<Name> segmentQname(
        ...
        int iterateStartPos = offset;
        int iterateOStart = start;
        
        StringTokenizer st = new StringTokenizer(qname, ".");
        while (st.hasMoreTokens()) {
            String subModule = st.nextToken();

            int nstart = iterateStartPos;
            int nstop = nstart + subModule.length();

            int iterateOStop = iterateOStart + subModule.length();
            
            Name retName = new Name(subModule, file, iterateOStart, iterateOStop);
            retName.setAstType(astType);

            retName.setLineStart(startLine);
            retName.setOffset(nstart); // CC start counting from 1

            retName.setLineEnd(endLine);
            retName.setEndOffset(nstop); // CC start counting from 1

            retName.setParent(parent);

            iterateStartPos = nstop + 1; // skip dot
            iterateOStart = iterateOStop + 1;
...
//            builder.redirectOutput(new File(parserLog));
            ...
            PPLogger.warning("Failed to start: " + pythonExe);
            return null;
        }
        return p;
    }
    ...
    public Node parseFile(String filename) {
        ...
        Process pythonProcessPrimary;
        Process pythonProcessSecondary;

        if (Analyzer.self.hasOption("python3") && python3Process != null) {
            pythonProcessPrimary = python3Process;
            pythonProcessSecondary = python2Process;
        } else {
            pythonProcessPrimary = python2Process;
            pythonProcessSecondary = python3Process;
        }
        Node nodePrimary = parseFileInner(filename, pythonProcessPrimary);
        if (nodePrimary != null) {
            return nodePrimary;
        } else if (pythonProcessSecondary != null) {
            Node nodeSecondary = parseFileInner(filename, pythonProcessSecondary);
            if (nodeSecondary == null) {
                PPLogger.warning("failed to parse: " + filename);
                Analyzer.self.failedToParse.add(filename);
                return null;
            } else {
                return nodeSecondary;
            }
        } else {
            PPLogger.warning("failed to parse: " + filename);
            Analyzer.self.failedToParse.add(filename);
    ...
    public Node parseFileInner(String filename, @NotNull Process pythonProcess) {
        ...
        while (!marker.exists()) {
            if (System.currentTimeMillis() - waitStart > TIMEOUT) {
                PPLogger.warning("Timed out while parsing: " + filename);
                cleanTemp();
                startPythonProcesses();
                return null;
            }
        ...
        if (json != null) {
            ...
            try {
                ...
                PPLogger.exception(e);
                PPLogger.severe(json);
                System.exit(1);
                return null;
            }
            return convert(map);
    ...
    private boolean sendCommand(String cmd, @NotNull Process pythonProcess) {
        try {
            ...
            PPLogger.warning("Failed to send command to interpreter: " + cmd);
            return false;
        }
    }
}
