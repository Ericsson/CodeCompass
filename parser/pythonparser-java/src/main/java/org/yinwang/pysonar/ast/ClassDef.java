...
public class ClassDef extends Node {
    ...
    public ClassDef(@NotNull Name name, List<Node> bases, Node body, String file, int start, int end) {
        ...
        addChildren(bases);
        Analyzer.self.ds.addClassDef(name, this);
    }

    public LinkedList<String> getConstructorHash() {
        LinkedList<String> ret = new LinkedList<>();
        for (Map.Entry<Name, FunctionDef> e : Analyzer.self.ds.getFd().entrySet()) {
            Name fdName = ((FunctionDef) e.getValue()).name;
            if (fdName.name.equals("__init__")) {
                if (fdName.containerBinding.equals(name.idHash)) {
                    ret.add(fdName.idHash);
                }
            }
        }
        return ret;
...
//        return classType.table.getConstructors();
//    }
    
    public List<Binding> getOwnAttributes(){
        return classType.table.getAttributes();
    }
    
    public List<Binding> getInheritedAttributes(){
        return classType.table.getInheritedAttributes();
    }
    ...
    public String toString() {
        return "(class:" + name.id + ":" + getStart() + " | BASES: " + bases + ")";
    }
