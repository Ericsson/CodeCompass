...
public class State {
    ...
    @Nullable
    public LinkedList<String> lookupDefinitionQname(String attr) {
        Set<Binding> b = lookupAttr(attr);
        LinkedList<String> ret = new LinkedList<>();

        if (b != null && !b.isEmpty()) {
            for (Binding bind : b) {
                ret.add(bind.qname + " " + bind.bodyStartLine);
            }
            return ret;
        } else {
            return null;
    ...
    public Set<Binding> getAllAttributes(){
        Set<Binding> attrs = new HashSet<>();
        attrs.addAll(getAttributes());
        attrs.addAll(getInheritedAttributes());
        return attrs;
    }

    public List<Binding> getAttributes() {
        List<Binding> attrs = new LinkedList<>();

        for (Map.Entry<String, Set<Binding>> e : table.entrySet()) {
            for (Binding b : e.getValue()) {
                if (b.kind == Binding.Kind.ATTRIBUTE && !(b.node instanceof Url)) {
                    attrs.add(b);
                }
            }
        }
    ...
    public List<Binding> getInheritedAttributes() {
        List<Binding> attrs = new LinkedList<>();
        ...
        for (State sup : supers) {
            attrs.addAll(sup.getAttributes());
            attrs.addAll(sup.getInheritedAttributes());
        }

        return attrs;
    }

    public List<Binding> getConstructors() {
        List<Binding> constructors = new LinkedList<>();

        for (Map.Entry<String, Set<Binding>> e : table.entrySet()) {
            for (Binding b : e.getValue()) {
                if (b.kind == Binding.Kind.CONSTRUCTOR) {
                    constructors.add(b);
                }
            }
        }
    ...
    public void mergePriorOther(State other) {
        for (Map.Entry<String, Set<Binding>> e2 : other.table.entrySet()) {
            Set<Binding> thisBinding = table.get(e2.getKey());
            Set<Binding> otherBinding = e2.getValue();

            if (thisBinding != null && otherBinding != null) {
                thisBinding.addAll(otherBinding);
//                cloneType(thisBinding, otherBinding);
            } else if (thisBinding == null && otherBinding != null) {
                table.put(e2.getKey(), otherBinding);
            }
        }
    }

    public void cloneType(Set<Binding> first, Set<Binding> second) {
        if (second.isEmpty()) {
            return;
        }
        ...
        for (Binding binding : second) {
            builtinType = binding.type;
            break;
        }
        for (Binding fb : first) {
            fb.type = builtinType;
        }
    }
    ...
    private boolean isDefinitionAttribute(String attrName) {
        if (!(type instanceof ClassType)) {
            return false; // not a class state
        }

        ClassType cls = (ClassType) type;
        Set<Binding> foundAttr = cls.table.lookupAttr(attrName);

        if (foundAttr == null) {
            foundAttr = cls.instanceAttributes.lookupAttr(attrName);
            if (foundAttr == null) {
                return true;
            }
        }
    ...
    public void insert(String id, @NotNull Node node, @NotNull Type type, Binding.Kind kind) {
        ...
        if (kind == Binding.Kind.ATTRIBUTE && isDefinitionAttribute(id)) {
            b.markAsDefinitionBinding();
        }
        update(id, b);
    }
    
    // create new binding and insert
    public void insertInstanceAttribute(String id, @NotNull Node node, @NotNull Type type, Binding.Kind kind, ClassType ct) {
        Binding b = new Binding(id, node, type, kind);
        if (type instanceof ModuleType) {
            b.setQname(type.asModuleType().qname);
        } else {
        ...
        update(id, b);
        ct.instanceAttributes.update(id, b);
    }
