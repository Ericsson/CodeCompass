...
public class FunType extends Type {
    ...
    // 0 if not constructor
    // init sequence number if constructor (the less the number the sooner 
    // the parser found the definition)
    ...
    public List<String> decorators = new LinkedList<>();
    ...
    /**
     * We resolve the decorators, but do not use the type of them.
     *
     * @param decorators list of decorators
     */
    public void setDecorators(@Nullable List<Node> decorators) {
        if (decorators == null) {
            return;
        }
        for (Node node : decorators) {
            this.decorators.add(node.name);
        }
    }
    ...
    public boolean equals(Object other) {
        if (other instanceof FunType) {
            ...
            // It has been restricted with 'fo.func.equals(func)'
            if (func != null && fo.func != null) {
                return fo.func.equals(func) && (fo.table.path.equals(table.path) || this == other);
            } else {
    ...
    public void markAsConstructor() {
        this.initSeq = Analyzer.self.getInitSeq();
    }
}
