...
function (Dialog, array, lang, style, ItemFileWriteStore, DataGrid, model) {
  return {
    ...
    getIconClass : function (path) {
      ...
      switch (extension) {
        case 'cc':
        case 'cpp':   return 'ext_cpp';
        case 'c':     return 'ext_c';
        case 'hpp':
        case 'h':     return 'ext_h';
        case 'cs':    return 'ext_cs';
        case 'o':     return 'ext_o';
        case 'css':   return 'ext_css';
        case 'html':  return 'ext_html';
        case 'java':  return 'ext_java';
        case 'class': return 'ext_class';
      }
    },
    ...
    /**
     * This function displays a dialog with a table which shows the possible
     * definitions or search results of a node.
     * @param {Array} elems Array of AstNodeInfo or SearchResultEntry objects
     * which are the possible definitions or text search results of the selected
     * node.
     * @param {Function} callback This callback function will be called when a row
     * is clicked in the table. The function is given a FileRange object.
     */
    buildAmbiguousRefPage : function (elems, callback) {
      if (elems.length === 0)
        return;
      ...
      if (elems[0] instanceof model.AstNodeInfo) {
        elems.forEach(function (nodeInfo) {
          ...
          fileRanges.push(nodeInfo.range);
        });
      } else {
        elems.forEach(function (searchResultEntry) {
          searchResultEntry.matchingLines.forEach(function (matchingLine) {
            store.newItem({
              id       : Math.random(),
              filename : searchResultEntry.finfo.path,
              position : 'line: ' + matchingLine.range.range.startpos.line
            });
            fileRanges.push(matchingLine.range);
          });
        });
      }
      ...
      var datagrid = new DataGrid({
        ...
        onRowClick : function (event) {
          dialog.hide();
          callback(fileRanges[event.rowIndex], event);
        }
      });
