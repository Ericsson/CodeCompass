...
function (declare, dom, topic, Button, CheckBox, Select, ContentPane, model) {
  
  function getAllFileTypes() {
    var fileTypes = [];
    for (var fileType in model.FileType)
      fileTypes.push(model.FileType[fileType]);
    return fileTypes;
  }
  ...
  return declare(ContentPane, {
    ...
    constructor : function() {
      ...
      topic.subscribe('codecompass/metrics', function (message) {
        that._currentFileId = message.fileId;
        
        var fileTypes = message.fileTypes || getAllFileTypes();
        var metricsTypes = message.metricsTypes || [
          that._metricsSelect1.get('value'),
          that._metricsSelect2.get('value')];
        
        that.loadMetrics(message.fileId, fileTypes, metricsTypes);
      });
    },
    ...
    postCreate : function () {
      ...
      for (var fileType in model.FileType) {
        var cb = function (fileType) {
          selectedFileTypes[model.FileType[fileType]] = true;
          return new CheckBox({
            checked : true,
            onChange : function (newValue) {
              if (newValue)
                ...
                delete selectedFileTypes[model.FileType[fileType]];
            }
          });
        }(fileType);
        
        var label = dom.toDom(fileType);
      ...
      var metricsButton = new Button({
        ...
        onClick : function () {
          var fileTypes = Object.keys(selectedFileTypes).map(function (x) {
            return parseInt(x);
          });
     ...
     * @param {Array} fileTypes Array of model.FileType items. The files under
     * fileId will be filtered on these file types.
     * @param {Array} metricsTypes Array of length 2 of model.MetricsType items.
