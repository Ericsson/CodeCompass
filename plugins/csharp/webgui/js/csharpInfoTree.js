...
function (model, viewHandler, util) {
  ...
  function loadReferenceNodes(parentNode, nodeInfo, refTypes) {
    ...
    var references = model.csharpservice.getReferences(
      nodeInfo.id,
      parentNode.refType);
              ...
              if (parentNode.refType === refTypes['Caller']) {
                res.push({
                  ...
                  getChildren : function () {
                    ...
                    var refCount = model.csharpservice.getReferenceCount(
                    ...
                    var calls = model.csharpservice.getReferences(
                      this.nodeInfo.id,
                      refTypes['This calls']);
  ...
  function loadFileReferenceNodes(parentNode) {
    ...
    var references = model.csharpservice.getFileReferences(
  ...
  var csharpInfoTree = {
    ...
    render : function (elementInfo) {
      ...
      if (elementInfo instanceof AstNodeInfo) {
        ...
        var props = model.csharpservice.getProperties(elementInfo.id);
        ...
        var refTypes = model.csharpservice.getReferenceTypes(elementInfo.id);
        for (var refType in refTypes) {
          var refCount =
            model.csharpservice.getReferenceCount(elementInfo.id, refTypes[refType]);
        ...
        var refTypes = model.csharpservice.getFileReferenceTypes(elementInfo.id);
        for (var refType in refTypes) {
          var refCount = model.csharpservice.getFileReferenceCount(
  ...
  viewHandler.registerModule(csharpInfoTree, {
