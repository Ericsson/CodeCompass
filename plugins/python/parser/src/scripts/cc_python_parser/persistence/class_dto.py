...
class ClassDeclarationDTO(DeclarationDTO, DocumentedDTO):
    class ClassMembersDTO:
        def __init__(self):
            self.methods: List[FunctionDTO] = []
            self.static_methods: List[FunctionDTO] = []
            self.attributes: List[VariableDTO] = []
            self.static_attributes: List[VariableDTO] = []
    ...
    def __init__(self, name: str, qualified_name: str, file_position: FilePosition, types: Set[str],
                 usages: List[UsageDTO], base_classes: Set[str], members: 'ClassDeclarationDTO.ClassMembersDTO', documentation: str):
