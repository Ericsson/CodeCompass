  ...
  const boost::program_options::variables_map& config_)
{
  ...
  auto configIt = config_.find("codechecker-url");
  ...
  configIt = config_.find("codechecker-runid");
  ...
  if (configIt != config_.end())
  {
    ...
    try {
      ...
      LOG(error) << "codecheckerrunid cannot be parsed as a number: " + sRunId;
      throw;
  ...
  configIt = config_.find("codechecker-runname");

  if (configIt != config_.end())
  {
    std::string runName = configIt->second.as<std::string>();

    LOG(warning) << "RUNNAME: " << runName;
    RunDataList runList;
