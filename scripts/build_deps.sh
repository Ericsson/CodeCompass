...
compile_CLAN()
{
  mkdir -p "$CCMP_DEPS/CLAN" || exit -1;
  cp -r . "$CCMP_DEPS/CLAN" || exit -1;
}
...
build_llvm()
{
  ...
  download http://llvm.org/releases/3.8.0/compiler-rt-3.8.0.src.tar.xz
  if [ $? -eq 0 ];
  then
    mv ./compiler-rt-3.8.0.src ./llvm-3.8.0.src/projects/compiler-rt
  fi
  compile llvm llvm-3.8.0.src lib/libLLVMCore.a --with-gcc-toolchain=$CCMP_DEPS CC=gcc CXX=g++ --with-python=python2 --disable-terminfo --enable-optimized
}
...
build_CLAN()
{
  local dlStat=0;
  (
    download http://md-mtas2.tsp.eth.ericsson.se/compass/deps/CLAN.tar.gz;
  )

  dlStat=$?;
  if [ $dlStat -eq 0 ] || [ $dlStat -eq 1 ]; then
    compile CLAN CLAN CLAN/cparse/doit;
  else
    echo_warning "CLAN is not supported!"
  fi
}
